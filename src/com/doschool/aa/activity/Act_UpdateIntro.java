...
public class Act_UpdateIntro extends Act_CommonOld {
	...
	public void initData() {
		ACTIONBAR_TITTLE = "版本特性";
		getActionBar().setSubtitle("您目前版本："+getResources().getString(R.string.versionName));
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		viewPager.setAdapter(new PagerAdapter() {
			...
			public View instantiateItem(ViewGroup container, int position) {
				ProgressImageView photoView =new ProgressImageView(Act_UpdateIntro.this, 200);
	...
	public class RefreshTopicListTask extends AsyncTask<Void, Void, Integer> {
		...
		protected Integer doInBackground(Void... params) {
			 ...
			 // 先读本地
            try {
                MJSONArray mjarr = new MJSONArray(SpMethods.loadString(getApplicationContext(), SpMethods.FEATURE_STR_LIST));
                urlList.clear();
            ...
            // 如果强制联网刷新 或者 数据集为空
                MJSONObject result = DoUserSever.CommonPicGet(1);
...
//                    DongSpMethods.saveLong(getApplicationContext(), DongSpMethods.FRIEND_LIST_UPDATE_TIME, System.currentTimeMillis());
                    SpMethods.saveString(getApplicationContext(), SpMethods.FEATURE_STR_LIST, mjarr.toString());
                    urlList.clear();
                    for (int i = 0; i < mjarr.length(); i++)
                    	urlList.add(mjarr.getMJSONObject(i).getString("site"));
