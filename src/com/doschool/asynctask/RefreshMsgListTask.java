 ...
 * @author 是我的海
 */
public class RefreshMsgListTask extends AsyncTask<Void, Void, Integer> {
	...
	public RefreshMsgListTask(PullToRefreshListView listview, ArrayList<Message> data, Adp_Standard adapter, boolean isMore) {
		super();
	...
	protected Integer doInBackground(Void... params) {
			...
			if(isMore==false)
				...
				lastBID=data.get(data.size()-1).mid;
			
			MJSONObject jobjReceive = DoUserSever.UserMsgListGet(
			...
			if(jobjReceive.getInt("code", 9)==0)
			{
				...
				if(isMore==false)
				{
					SpMethods.saveLong(listview.getContext(), SpMethods.MSG_TIME_LIST, System.currentTimeMillis());
					SpMethods.saveString(listview.getContext(), SpMethods.MSG_STR_LIST, jarrayData.toString());		
					data.clear();
				}
				for (int i = 0; i < jarrayData.length(); i++)
					data.add(new Message(jarrayData.getMJSONObject(i)));
				return 0;
			}
			else 
				//联网刷新失败
