 ...
 * @author 是我的海
 */
public class RefreshRequestListTask extends AsyncTask<Void, Void, Integer> {
	...
	public RefreshRequestListTask(PullToRefreshListView listview, ArrayList<FriendRequest> data, Adp_Standard adapter, boolean isMore) {
		super();
	...
	protected Integer doInBackground(Void... params) {
			...
			if(isMore!=false && data.size()>0)
				lastBID=data.get(data.size()-1).requestId;
			
			MJSONObject jobjReceive = DoUserSever.UserApplyListGet(DoschoolApp.thisUser.personId, lastBID, 10);
			if(jobjReceive.getInt("code", 9)==0)
			{
				...
				if(isMore==false)
				{
					SpMethods.saveLong(listview.getContext(), SpMethods.REQ_TIME_LIST, System.currentTimeMillis());
					SpMethods.saveString(listview.getContext(), SpMethods.REQ_STR_LIST, jarrayData.toString());	
					data.clear();
				}
				for (int i = 0; i < jarrayData.length(); i++)
					data.add(new FriendRequest(jarrayData.getMJSONObject(i)));
				return 0;
			}
			else 
				//联网刷新失败
